...
| Trajectory compaction | `ICompactionStrategy` | `NullCompactionStrategy` (omission-note *view*); opt in to the `AiCompactionStrategy` incremental *fold* via `WithAiCompaction(modelClient)`, or a custom strategy via `WithCompactionStrategy<T>()` | ↑ same |
