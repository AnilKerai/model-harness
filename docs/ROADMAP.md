...
- [x] **Incremental fold compaction (rolling summary)** — `AiCompactionStrategy` is now an incremental **fold**, not a view: it summarises only the newly-evicted slice on top of the prior summary, so cost stays flat as the run grows (was: re-summarise the entire growing evicted head every turn). Delivered:
  - **Fold-capable port** — `ICompactionStrategy.SummariseAsync(...)` became `CompactAsync(CompactionRequest, ct) → Task<CompactionResult>`. `CompactionRequest` carries the **typed** `EvictedSteps` (not pre-rendered text — so a strategy can compact structurally), the full `AgentState`, the `PriorSummary`, and the token budget; it's a record so new inputs can be added without breaking implementers. `CompactionResult` carries `InjectedText` + a nullable `UpdatedSummary` (null ⇒ stateless view, the `NullCompactionStrategy` default; non-null ⇒ fold) + `Usage`/`Cost`. A framework extension point registered via the new `WithCompactionStrategy<T>()` builder method (or the `WithAiCompaction(model)` shortcut). Breaking change for the (niche) port.
