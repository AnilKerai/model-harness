...
- [x] **Incremental fold compaction (rolling summary)** — `AiCompactionStrategy` is now an incremental **fold**, not a view: it summarises only the newly-evicted slice on top of the prior summary, so cost stays flat as the run grows (was: re-summarise the entire growing evicted head every turn). Delivered:
  ...
  - **Deferred (layers on top, none blocked by this):** deterministic structured clearing (truncate bulky old tool results, zero model cost) as a cheap pre-fold layer / possible free default; the `compact_context` span + `gen_ai.conversation.compacted=true` (the span-aware tracer it hangs off is now delivered); an anti-thrash guard; a preservation set beyond head-only eviction; defaulting the AI fold on in the Standard harness and a zero-arg `WithAiCompaction()` reusing the registered `IModelClient`. Sources: MemGPT arXiv 2310.08560 §Queue Manager; LangGraph memory / langmem summarization; Anthropic context-editing + compaction docs; "Beyond Compaction" arXiv 2606.11213; Manus context-engineering notes.
