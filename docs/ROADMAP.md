...
- [ ] **Nested `gen_ai.*` span tree via a span-aware `ITracer` port** — `OpenTelemetryTracer` already emits via native `ActivitySource`/`Meter`, but it opens a single `agent.task` span and hangs `model_call`/`tool_call`/`sensor`/`guide` as *events* on it, under our own attribute names. The industry norm — OTel GenAI semantic conventions (now in `open-telemetry/semantic-conventions-genai`; what `Microsoft.Extensions.AI` / Semantic Kernel / Agent Framework emit) — is a **nested span tree**: `invoke_agent` → `chat {model}` (Client) / `execute_tool {name}` (Internal), with `gen_ai.*` attributes and the metrics `gen_ai.client.token.usage` (`{token}`) + `gen_ai.client.operation.duration` (`s`). This is *not* "add OTel" (we have it) but "flat events → nested `gen_ai.*` spans". Plan:
  - **Make the `ITracer` port span-aware** — the current post-hoc `LogXxx` methods fire *after* an operation returns, so a tracer can only add a zero-duration event, never bracket a real span. Add a begin/end (scope) shape so the loop opens a scope around each model/tool call and every registered tracer materialises it its own way: `OpenTelemetryTracer` → a child `Activity` with correct duration; `ConsoleTracer` → an indented block; a custom debug tracer → whatever. **Keep the multi-tracer port + `CompositeTracer`** — register native OTel *and* a debug tracer together; do not move spans into the loop and sideline the port. Breaking change for external `ITracer` implementers (same class as the `turn`-index change).
  - **Adopt `gen_ai.*` vocabulary** — `gen_ai.provider.name` (`anthropic`/`azure.ai.openai`), `gen_ai.usage.input_tokens`/`output_tokens`, `gen_ai.response.finish_reasons`, `gen_ai.tool.name`/`call.id`; tool `IsError` → span error + `error.type`. OTel GenAI is still "Development"/experimental — pin semconv versions; optionally dual-emit OpenInference (`openinference.span.kind`, `llm.*`) for Phoenix/Langfuse.
  - **Sensors & guides have no standard span** — scoring sensors (`CriticSensor`, `ProgressCheckSensor`) → the `gen_ai.evaluation.result` event and/or OpenInference `EVALUATOR`; content sensors (`PromptInjectionSensor`, `PiiRedactionSensor`, `TaintTrackingSensor`) → OpenInference `GUARDRAIL`; carry our hookpoint/action as namespaced `harness.sensor.*`. Guides stay custom `harness.guide.*` (already emitted via `GuideContribution`), except `MemoryGuide` → a real `search_memory`/`RETRIEVER` span with `gen_ai.memory.record.count`.
  - **Content capture** (prompt/response bodies) behind an `EnableSensitiveData`-style flag, default off — matches `Microsoft.Extensions.AI` (`OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT`); we already log counts only. **Cost**: OTel GenAI has no cost attribute (backends compute it from tokens) — keep our computed cost under `harness.cost`.
  - No new package dependency — `ActivitySource`/`Activity`/`Meter` are shared-framework types (Infrastructure already uses them with no `System.Diagnostics.DiagnosticSource` package ref), so a Framework-level `ActivitySource` is free and keeps the "only `DI.Abstractions`" package rule.
  - **Enables the compaction-fold item** (Guide pattern) — the `compact_context` span + compaction-spend attribution defined there is the instrument to watch while rebuilding compaction. Sources: OTel GenAI spans/agent-spans/metrics/events docs; OpenInference semantic conventions; `Microsoft.Extensions.AI` `OpenTelemetryChatClient`.
