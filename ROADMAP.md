...
- [ ] Smarter compaction via summarisation — current compaction discards oldest steps and prepends a bare omission note ("N steps were omitted"), losing signal the model may still need. Replace with a summarisation strategy: when steps must be evicted, call a lightweight model (Haiku-class) to produce a concise prose summary of the dropped segment ("customer identified as Pro tier, known issue KI-001 found, manual reset suggested") and inject that in place of the raw omission note. Design notes: introduce an `ICompactionStrategy` port on `TrajectoryGuide` with a `NullCompactionStrategy` default (current truncation behaviour, no extra model call) and an `AiCompactionStrategy` implementation in `Infrastructure.Anthropic`; the strategy receives the steps being evicted and the remaining token budget and returns a summary string; the AI strategy must fail open (fall back to the truncation note if the model call fails or times out); cost from the compaction call should be tracked separately from the main agent cost so operators can see compaction overhead in traces.
